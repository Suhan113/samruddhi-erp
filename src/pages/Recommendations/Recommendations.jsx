...
export default function Recommendations() {
  ...
  // Filtered soil reports available for recommendation (include current one if editing)
  const availableSoilTests = useMemo(() => {
    const prescribedIds = recommendations
      .filter(r => r.id !== editingId)
      .map(r => r.soil_test_id);
    return soilTests.filter(st => !prescribedIds.includes(st.id));
  }, [soilTests, recommendations, editingId]);
  ...
  const addMaterialRow = () => {
    setSelectedMaterialsList([
      ...
      { material_name: "", category: "Organic", quantity_per_plant: 0, unit: "Kg", remarks: "" }
    ]);
  };
  ...
  const handleEdit = async (rec) => {
    ...
    try {
      ...
      console.error("Error loading recommendation materials for edit", err);
      setSelectedMaterialsList([]);
    }
    ...
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        ...
        <button
          ...
          onClick={() => { setEditingId(null); setForm(initialForm); setSelectedMaterialsList([]); setShowModal(true); }}
      ...
      {/* Warning */}
      {availableSoilTests.length === 0 && !loading && recommendations.length === 0 && (
                      ...
                      <div key={rm.id} style={materialItemStyle}>
                        <div>
                          ...
                          <div style={{ fontSize: "11px", color: "var(--text-muted)" }}>
                            Category: {rm.category} • {rm.quantity_per_plant} {rm.unit || "Kg"} / tree
                          </div>
                        </div>
          ...
          <div className="card-premium animate-fade-in" style={modalCardStyle}>
            <div style={modalHeaderStyle}>
              <h3 style={{ fontSize: "20px", fontWeight: 700 }}>
                {editingId ? "Edit Fertilizer Recipe" : "Formulate Fertilizer Recipe"}
              </h3>
            ...
            <form onSubmit={handleSubmit} style={modalFormStyle}>
              <div style={modalFormGridStyle}>
                ...
                <div style={{ gridColumn: "1 / -1", borderTop: "1px solid var(--border)", paddingTop: "16px", marginTop: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                    <h4 style={{ fontSize: "14px", fontWeight: 700 }}>Prescribe Input Materials (Per Plant Quantities)</h4>
                  ...
                  {selectedMaterialsList.length === 0 ? (
                    <div style={{ padding: "16px", textAlign: "center", background: "var(--bg-app)", borderRadius: "8px", border: "1px dashed var(--border)", fontSize: "13px", color: "var(--text-muted)" }}>
                      No materials selected. Click '+ Append Material Line' to begin prescribing rows per plant.
                    </div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <div style={modalActionsStyle}>
                ...
                <button type="submit" className="btn-primary">
                  {editingId ? "Update Recommendation" : "Generate Recommendation"}
                </button>
              </div>
            </form>
          </div>
